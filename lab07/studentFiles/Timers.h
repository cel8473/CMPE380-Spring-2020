...
/************** Write your timing macros here **************/
  ...
  ****************************************************************************/
  #define DECLARE_REPEAT_VAR(V) int V; 
  /* R - repeat value, V - variable */  
  #define BEGIN_REPEAT_TIMING(R,V) for(V = 0; V < R; V++){ 
  ...
  ****************************************************************************/
  #define PRINT_TIMER(A)  				\
	{ 	if(A.State == 1){ STOP_TIMER(A)}	\
		printf("Timer = %g\n",(double) A.Elapsed/(double) CLOCKS_PER_SEC);	\
	}
  #define PRINT_RTIMER(A,R)				\
	{	PRINT_TIMER(A)				\
		double j = ((double) A.Elapsed/((double)CLOCKS_PER_SEC))/(double)R;\
		printf("Timer divided by R = %g\n", j);	\
	}
