...
int main(int argc, char *argv[])
  {
	...
	printf("**********************************************************\n");
	printf("This is the hw8c_opt3\n");
	DECLARE_REPEAT_VAR(dataVar)
	DECLARE_REPEAT_VAR(calcVar)
	DECLARE_TIMER(Dtimer)
	DECLARE_TIMER(Ctimer)
   ...
   if (1 != argc)
	...
	START_TIMER(Dtimer)	
	BEGIN_REPEAT_TIMING(DATAREPS, dataVar)
	...
	END_REPEAT_TIMING
	STOP_TIMER(Dtimer)
	PRINT_RTIMER(Dtimer,DATAREPS)
	RESET_TIMER(Dtimer)
   /* Insert start calc timing */
	START_TIMER(Ctimer)
	BEGIN_REPEAT_TIMING(CALCREPS, calcVar)
...
void CalculateCoefficients(LinearFit *DataSet, double *A, double *B)
  {
   ...
   for (lcv=0; lcv < DataSet->NextElement; lcv+=2)
    {
      T_XX += DataSet->Data_X[lcv] * DataSet->Data_X[lcv];
      T_XY += DataSet->Data_X[lcv] * DataSet->Data_Y[lcv];
      T_X  += DataSet->Data_X[lcv];
      T_Y  += DataSet->Data_Y[lcv];
